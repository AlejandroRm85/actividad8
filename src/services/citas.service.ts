...
import { getJuzgadoById } from './juzgados.service';
import { Cita, NuevaCita, supabase } from './supabase';
...
export const getHorariosOcupados = async (
  ...
  const juzgado = await getJuzgadoById(juzgadoId);
  const { data, error } = await supabase
    .from('citas')
    .select('hora_cita')
    .eq('juzgado_id', juzgadoId)
    .eq('fecha_cita', fecha)
    .eq('estado', 'programada');

  if (error) throw error;

  const citasPorHora = (data ?? []).reduce<Record<string, number>>((acc, row: { hora_cita: string }) => {
    const hora = row.hora_cita.substring(0, 5);
    acc[hora] = (acc[hora] ?? 0) + 1;
    return acc;
  }, {});

  return Object.entries(citasPorHora)
    .filter(([, total]) => total >= juzgado.capacidad_por_hora)
    .map(([hora]) => hora);
};
